...
public class SmartRefreshLayout extends ViewGroup implements RefreshLayout, NestedScrollingParent/*, NestedScrollingChild*/ {
    ...
    protected int     mReboundDuration         = 650;//回弹动画时长
    ...
    protected float mHeaderMaxDragRate = 2f;  //最大拖动比率(最大高度/Header高度)
    ...
    public SmartRefreshLayout(Context context, AttributeSet attrs) {
        ...
        mReboundInterpolator = new SmartUtil(SmartUtil.INTERPOLATOR_DECELERATE);
...
//                            resetStatus();
    ...
    @Override
    @Deprecated
    public boolean autoRefresh(final int delayed) {
        return autoRefresh(delayed, mReboundDuration, 1f * ((mHeaderMaxDragRate / 2 + 0.25f) * mHeaderHeight) / (mHeaderHeight == 0 ? 1 : mHeaderHeight), false);
    }
    ...
    public boolean autoRefreshAnimationOnly() {
        return autoRefresh(mAttachedToWindow ? 0 : 400, mReboundDuration, 1f * ((mHeaderMaxDragRate / 2 + 0.25f) * mHeaderHeight) / (mHeaderHeight == 0 ? 1 : mHeaderHeight), true);
    }
